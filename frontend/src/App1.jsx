...
function App() {
  ...
  }, [documents, documentPage, documentPageSize, documentSearch, documentSort, documentStatusFilter]);
  ...
  }, [documents, xmlConvertingId, xmlPage, xmlPageSize, xmlSearch, xmlSort, xmlStatusFilter]);
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      xmlCount: documents.filter((document) => document.has_xml).length,
    };
  }, [documents, tocEntries]);
  ...
  async function handleDeleteDocument(document) {
    if (!document) return;
    if (activeJob?.document_id === document.id || document.status === "processing") {
      setNotice({ type: "error", message: "This document is processing and cannot be deleted yet." });
      return;
    }
    ...
    <div className="app-shell">
      ...
      <main className="main">
        <Topbar
          ...
          onRefresh={() => refreshDocuments(selectedDocument?.id)}
          onUpload={() => fileInputRef.current?.click()}
        />

        <SystemStatus activeJob={activeJob} documents={documents} />
...
function Topbar({ activeView, selectedDocument, onRefresh, onUpload }) {
  const isXmlView = activeView === "xml";
  return (
    <header className="topbar">
      <div className="topbar-copy">
        <p className="eyebrow">Operations Console</p>
        <h1>{isXmlView ? "PDF to XML Conversion" : "TOC and Hyperlinking"}</h1>
        <span>
          {selectedDocument
            ? `${isXmlView ? "XML workspace focused on" : "Currently focused on"} ${selectedDocument.filename}`
            : "Upload a source document to begin processing."}
        </span>
      </div>
...
function UploadCard({
  ...
  dropDescription = "or open the file picker to add it to the processing queue.",
  tags = ["PDF only", "100 MB max", "EICAS OCR"],
}) {
  return (
    <section
  ...
  return (
    <section className="document-panel" aria-labelledby="selected-document-title">
      <div className="selected-document-hero">
        ...
        <StatusPill status={document?.status || "idle"} />
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex min-h-11 items-center gap-2 rounded-[8px] border border-slate-200 bg-white px-3 text-sm font-bold text-slate-600 shadow-sm">
            ...
            >
              ...
              <option value="completed">Ready to view</option>
...
function DocumentTableRow({
  ...
  const showWorkflow = isActiveJob || document.status === "processing";
...
function DocumentRowActions({ document, isActiveJob, processing, deleting, onProcess, onDelete }) {
  const isRunning = isActiveJob || document.status === "processing";
  const canDownload = document.status === "completed" && document.has_output;
...
function RowWorkflow({ document, isActiveJob, jobStatus }) {
  const currentStatus = isActiveJob ? jobStatus?.status : document.status;
  const ready = document.has_output || document.status === "completed" || currentStatus === "finished";
  const queued = Boolean(isActiveJob && currentStatus);
  const running = document.status === "processing" || currentStatus === "started";
  const steps = [
  ...
  return (
    <section className="xml-page" id="xml-workspace" aria-labelledby="xml-title">
      <UploadCard
        ...
        dropDescription="Upload files here to build structured XML records without running TOC processing."
      ...
      <div className="xml-command-panel">
        <div className="selected-document-hero">
          ...
          <StatusPill status={document?.has_xml ? "finished" : "uploaded"} />
        </div>
        ...
        <div className="xml-controls">
          ...
          <button className="primary-button" type="button" disabled={!document || converting} onClick={() => onConvert(document.id)}>
    ...
    <section className="table-panel xml-documents-panel" aria-labelledby="xml-documents-title">
      ...
      <div className="xml-document-table">
        <div className="xml-document-head">
          <SortHeader label="Sr No / Document" sortKey="filename" sort={sort} onSort={onSort} />
          <SortHeader label="XML Status" sortKey="xml_status" sort={sort} onSort={onSort} />
          <SortHeader label="Pages" sortKey="page_count" sort={sort} onSort={onSort} />
...
function XmlDocumentRow({ document, index, isSelected, converting, convertingAny, deleting, onSelect, onConvert, onDelete }) {
  return (
    <div
      className={`xml-document-row ${isSelected ? "selected" : ""}`}
      role="button"
      tabIndex={0}
      onClick={() => onSelect(document.id)}
      onKeyDown={(event) => {
    ...
    >
      ...
      <XmlStatusPill status={document.xml_status} />
      <span className="table-value">{document.page_count ?? "-"}</span>
      <UserStamp name={document.created_by || "System"} date={document.created_at} />
      <UserStamp name={document.updated_by || "System"} date={document.updated_at} />
      <XmlRowActions
...
function XmlRowActions({ document, converting, convertingAny, deleting, onConvert, onDelete }) {
...
function WorkflowStatus({ document, jobStatus, processing }) {
  const steps = [
    { label: "Uploaded", done: Boolean(document), active: false },
    { label: "Queued", done: Boolean(jobStatus), active: processing && jobStatus?.status === "queued" },
    { label: "Processing", done: document?.status === "completed", active: processing && jobStatus?.status === "started" },
    { label: "Ready", done: Boolean(document?.has_output), active: false },
  ];
...
function xmlStatusForDocument(document, convertingId) {
  if (convertingId === document.id) return "converting";
  return document.has_xml ? "converted" : "pending";
}
